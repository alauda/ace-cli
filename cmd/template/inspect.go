...
// NewInspectCmd creates a new template inspect command.
func NewInspectCmd(alauda client.APIClient) *cobra.Command {
	inspectCmd := &cobra.Command{
		Use:   "inspect NAME",
		Short: "Inspect an app template",
		Long:  ``,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) != 1 {
				return errors.New("template inspect expects NAME")
			}
...
func doInspect(alauda client.APIClient, name string) error {
	...
	result, err := alauda.InspectAppTemplate(name)
	if err != nil {
		return err
	}
